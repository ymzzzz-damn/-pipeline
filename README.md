...
项目简介 (Introduction)
本项目是一个专为边端设备（如 NAS、IoT 网关）设计的本地化、可逆且语义保真的数据脱敏系统。
针对传统脱敏方法（如统一打星号、假名化）导致的下游 NLP 任务（如情感分析、关系抽取）失效问题，本项目提出了一种基于局部可预测性（Predictability, P值）与结构稳定性（Stability, S值）的四象限动态分级脱敏模型。
...
上下文感知消歧 (Context-aware Role Disambiguation)：`ContextAnalyzer` 模块，根据实体上下文（如“医生”、“患者”）分配 Role 标签，解决同名实体的语义冲突问题。
...
请确保您的本地目录严格遵循以下结构，否则预处理脚本将无法找到对应源文件：

```text
您的项目根目录/
├── downloads/
│   ├── adult/
│   │   └── adult.data                  # 从 UCI Adult 数据集提取的核心数据文件
│   └── disease/
│       ├── processed.cleveland.data    # 从 UCI Heart Disease 提取的 4 个子数据集
│       ├── processed.hungarian.data
│       ├── processed.switzerland.data
│       └── processed.va.data
│
├── input_processen/
│   └── emails.csv                      # 从 Kaggle 下载的 Enron 邮件合集文件
│
├── input_processcn/
│   ├── weiboNER_2nd_conll.train        # 微博 NER 训练集文件
│   └── train.char.bmes                 # 简历 NER 训练集文件 (BMES格式)
